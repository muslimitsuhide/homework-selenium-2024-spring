...
class CenterCommercePageLocators(BasePageLocators):
    ...
    CATALOG_SOURCE_TYPE_FEED = (
        By.XPATH, 
        "//div[@data-entityid='url']"
    )
    CATALOG_SOURCE_TYPE_MARKETPLACE = (
        By.XPATH, 
        "//div[@data-entityid='marketplace']"
    )
    CATALOG_SOURCE_TYPE_FILE = (
        By.XPATH, 
        "//div[@data-entityid='file']"
    )
    ...
    FEED_CATEGORY_SELECT = (
        By.XPATH, 
        "//h2[contains(text(),'Категория фида')]/following-sibling::div//input[@role='combobox']"
    )
    FEED_FILE_INPUT = (
        By.XPATH, 
        "//h2[contains(text(),'Файл фида')]/following-sibling::div//input[@type='file']"
    )
